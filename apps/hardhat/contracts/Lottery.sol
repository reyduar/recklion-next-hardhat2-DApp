...
contract Lottery is ERC20, Ownable {
    ...
    function precioTokens(uint256 _numTokens) internal pure returns (uint256) {
        return _numTokens * (1 ether); // este precio puede ser un valor de decision del usuario
    }
    ...
    function balanceEthersSC() public view returns (uint256) {
        return address(this).balance / 10 ** 18; // para retorna todo en Ether
    }
    ...
    function registrar() internal {
        // esta es una funcion factory que permite crear otros SC de forma automatica
...
contract mainERC721 is ERC721 {
    ...
    constructor() ERC721("Lottery", "NELO") {
    ...
    function safeMint(address _propietario, uint256 _boleto) public {
        require(
            msg.sender == Lottery(direccionLoteria).usersInfo(_propietario),
...
// Smart Contract de boletos NFTs
